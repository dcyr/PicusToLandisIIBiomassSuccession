...
clusterN <- 10 ### this script is I/O limited, I'm not even sure parallelizing is useful here
...
for (a in seq_along(folderNames)) {
    ...
    # for (p in names(picusif (exists("picusOutputsDF")) {
    #         picusOutputsDF  <- rbind(picusOutputsDF, do.call("rbind", picusOutputs[[s]]))
    #     } else {
    #         picusOutputsDF <- do.call("rbind", picusOutputs[[s]])
    #     }
    #     
    # }
...
endTime-beginTime
# foo <- read.csv(paste0(processedOutputDir, "/picusOutputsDF_", areaCode, ".csv"))
